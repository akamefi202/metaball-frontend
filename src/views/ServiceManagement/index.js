...
const ContentManagement = () => {
  ...
  const { loading, error, service, count } = useSelector(
    (state) => state.service
  );
  ...
  const onAddContent = () => {
    navigate(`/admin/service_management/add/${tabKey}`);
  };

  const onUpdateContent = (id) => {
    navigate(`/admin/service_management/update/${tabKey}/${id}`);
  };
  ...
  }, [fetchAllServices, tabKey, currentPage]);
              ...
              <CardFooter className="py-4">
                <nav aria-label="...">
                  ...
                  >
                    ...
                    <PaginationItem
                      disabled={currentPage > service.count / TABLE_PAGE_LIMIT}
                    >
                      <PaginationLink
                        onClick={() => setCurrentPage(currentPage + 1)}
                      >
